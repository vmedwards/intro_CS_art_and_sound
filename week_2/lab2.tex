...
\begin{center}
  {\Large {\bf Lab 2: Functions that More General Shapes}}
...
\begin{itemize}
  ...
  \item Create a new folder using your terminal by typing: \texttt{mkdir lab2}
  \item Move into that folder using your terminal by typing: \texttt{cd lab2}
  \item Using your terminal, confirm your current directory by typing: \texttt{pwd}.
\end{itemize}
...
Now we are going to consider the code organization and good practices.
...
\item Every function should have a comment that describes what the function does
\item At least one inline comment if there are more than 3 lines of code in the function
\item A comment indicating where the main code starts
\item Main code the program will run
\end{enumerate}
Check out the \texttt{turtle.speed()} function to see how you can speed up the drawing of your figures.
Don't forget to use the \texttt{turtle.exitonclick()} function at the end of your file 
...
\subsection{Write a function called \texttt{goto}}
Your goto function should take in two parameters: \texttt{x} and \texttt{y}. This will allow the turtle to go position x and y, no matter where it currently is on the canvas.

You can write this function in three steps:
\begin{enumerate}
\item Call the turtle \texttt{penup()} function to make the turtle no longer "drag a virtual pen underneath it as it moves". In other words, moving the turtle won't result in drawing anything to the on-screen canvas (i.e. your canvas stays white as the turtle moves).
\item Call the turtle \texttt{goto(x, y)} function to have the turtle jump to the new (x, y) coordinate given by the function parameter values.
\item Call the turtle \texttt{setheading(angle)} function to have the turtle face east.
\item Call the turtle \texttt{pendown()} function so that the turtle will once again draw as it moves around.
\end{enumerate}

Some more things to add to your function:
\begin{itemize}
\item To help you understand how your \texttt{goto} function works, make this print statement the first line of code in your goto function: \texttt{print('goto(): going to', x, y)}.
\item After that print statement, add this one: \texttt{print('goto(): before move, turtle at', turtle.position())}. Recall that the \texttt{position()} turtle function gives us the turtle's CURRENT (x, y) position.
\item Make the last line of the function a similar print statement: \texttt{print('goto(): after move, turtle now at', turtle.position())}.
\end{itemize}

\subsection{Test your \texttt{goto} function}
In main code at the bottom of your file, call your \texttt{goto} function (move the turtle to a position of your choice), then have the turtle draw a triangle there. Repeat this a few times with different positions.
Remember: In main code, you want to call YOUR goto function, NOT the turtle's. So you should write commands like \texttt{goto(140, 0)} rather than \texttt{turtle.goto(140, 0)}.

\textbf{Note: Why \texttt{penup} and \texttt{pendown}}
Before continuing, comment out both the \texttt{penup()} and \texttt{pendown()} commands in your \texttt{goto()} function then run your program. What happens? Do you see why they are needed in \texttt{goto}?
Remember to uncomment them before continuing on to the next task!
...
Write a new function called \texttt{triangle2} that has the following signature:
...
def triangle2(x, y, scale):
    ...
    # Your code goes here
\end{lstlisting}
Your \texttt{triangle2} function should move the turtle to the given (x, y) position and draw a triangle with a side length equal to 100 pixels when the scale parameter is 1.
...
Comment out any existing function calls in the main code that are not \texttt{turtle.speed()}, or \texttt{turtle.exitonclick()}. Test \texttt{triangle2} by calling it 3 times: with scale = 1 (standard size), scale = 2 (double standard size), and scale = 3 (triple standard size).
...
Next, comment out existing main code except for \texttt{turtle.speed()} and \texttt{turtle.exitonclick()}.
...
The second and last step is to make the triangle stack re-sizable. Do this by multiplying the offsets and scales by the parameter scale.
...
\item Draw the 1st stack at scale 1, the 2nd at scale 1/2 and the third at scale 1/3
  \item Draw at these 3 positions \texttt{(x, y) = (-200, 0), (x, y) = (0, 0)}, and \texttt{(x, y) = (100, 0)}
\end{itemize}
...
\section{Submission}
Please upload your \texttt{lab2.py} code to the google class room by midnight on \textbf{Friday April 10, 2026}.
