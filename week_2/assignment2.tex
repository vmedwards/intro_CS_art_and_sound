...
\section{Develop a design for your scene}
...
If you did not already do so, in your \textcolor{blue}{\texttt{goto}} function in \texttt{shapelib.py}, add a call to the \texttt{setheading} turtle function so that the turtle always faces east when you call \textcolor{blue}{\texttt{goto}} (We are using "standard mode").

This may be helpful when you create your scene because you will know the turtle is always facing east when it moves to draw a new shape.
...
\textbf{Test out each of your functions one-by-one as you make them.} This means calling your two shapes in main code at the bottom of your \texttt{shapelib.py} file several times to draw each in a few different positions and sizes.
...
Just like \textcolor{blue}{\texttt{triangleStack}} from lab, your compound shape functions should take at least three parameters: (x position, y position, scale).
...
\item Your shapes should draw properly at any scale, location, or turtle orientation.
\end{itemize}
...
Your final scene will rely on your shape library.
Make a new file called \texttt{main.py} and do the following:
\begin{itemize}
\item put your name, date, and purpose in a header comment
\item include the commands: \texttt{import turtle} and on a new line \texttt{import shapelib}
\item Define a function called \textcolor{blue}{\texttt{myScene}}: which will use the simple and compound shapes that you have made — drawing them in different positions and sizes. You can also use color and the random module to make your scene more complex and interesting.
\item to call functions from your shapelib use: \texttt{shapelib.triangle(100)}. Where the dot is tellin the python interperter to look in the file shapelib and grab the correct function
\item You may notice that if you have main code in \texttt{shapelib.py} this will also run when you run \texttt{main.py}. For now comment out any main function calls in \texttt{shapelib.py}.
\end{itemize}

When you are happy with your scene, take a screenshot. This is \textbf{Required Image 6} that you should include in your report.
